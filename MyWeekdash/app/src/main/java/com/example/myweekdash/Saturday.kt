...
class Saturday : AppCompatActivity() {
    ...
    private fun save(){
        val saturdayText = input.text.toString()
        val sp = getSharedPreferences("Data", MODE_PRIVATE)
        val editor = sp.edit()
        editor.putString("Saturday", saturdayText)
        editor.apply()
    }

    private fun load(){
        val saturdayText: String
        val sp = getSharedPreferences("Data", MODE_PRIVATE)
        saturdayText = sp.getString("Saturday", String()).toString()
        input.setText(saturdayText)
    }
}