...
class Friday : AppCompatActivity() {
    ...
    private fun save(){
        val fridayText = input.text.toString()
        val sp = getSharedPreferences("Data", MODE_PRIVATE)
        val editor = sp.edit()
        editor.putString("Friday", fridayText)
        editor.apply()
    }

    private fun load(){
        val fridayText: String
        val sp = getSharedPreferences("Data", MODE_PRIVATE)
        fridayText = sp.getString("Friday", String()).toString()
        input.setText(fridayText)
    }
}